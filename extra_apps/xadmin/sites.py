...
class MergeAdminMetaclass(type):
    """ 
    用来生成 admin view class 的原类。 目前该原类没有做什么特殊的工作。接下来的版本该原类可能会给 admin view class 注入一些公共的属性。 
    """
...
class AdminSite(object):
    """ 
    xadmin最核心的类，管理整个xadmin站点的所有注册内容。一般一个管理站点只有一个 ``AdminSite`` 实例，
    该实例主要完成以下工作: 
        * 注册管理所有 xadmin 需要的信息 
        * 创建 ``admin view class`` 
        * 注册 django urls 
    ...
    def __init__(self, name='xadmin'):
        ...
        self._registry_views = []       #: 保存所有 Model Base Admin View Class
    ...
    def register_modelview(self, path, admin_view_class, name):
        ...
        具体内容可以参看 `~xadmin.views.base.ModelAdminView`。 
        举例:: 
    ...
    def register_view(self, path, admin_view_class, name):
        ...
        关于 Admin View 具体内容可以参看 :class:`~xadmin.views.base.BaseAdminView`。 
        举例:: 
    ...
    def register_plugin(self, plugin_class, admin_view_class):
        ...
        关于 Admin Plugin 具体内容可以参看 :class:`~xadmin.views.base.BaseAdminPlugin`。 
        举例:: 
    ...
    def register(self, model_or_iterable, admin_class=object, **options):
        ...
        注册需要管理的 Model， 或是注册某 AdminView 的 OptionClass 
            ...
            :param admin_class: 当 model_or_iterable 为 Model 时，该参数为 ModelAdmin；
        model_or_iterable 为 AdminView 时 ，该参数为 OptionClass 
        关于 Admin Plugin 具体内容可以参看 :class:`~xadmin.views.base.BaseAdminPlugin`。 
        举例:: 
    ...
    def admin_view(self, view, cacheable=False):
        ...
        为当前 ``AdminSite`` 的所有 View 提供的 Decorator。主要是功能是使用 `AdminSite.has_permission` 方法
        来判断当前用户是否有权限访问该 ``AdminSite``，如果没有，转到登陆页面。
        通常情况下会在 `AdminSite.get_urls` 方法中使用该方法 
            :param cacheable: 默认情况下，所有的 AdminView 会通过 ``never_cache`` 标记成不做缓存，
        如果确实需要缓存，可以设置 cacheable=True 
        """
    ...
    def get_plugins(self, admin_view_class, *option_classes):
        ...
        获取 plugins 首先根据该 AdminViewClass 及其所有的集成类在已经注册的插件中找到相应的插件类。
        然后再使用第二个参数的 OptionClass 拼成插件类。 
        """
        from xadmin.views import BaseAdminView
        plugins = []
    ...
    def create_admin_view(self, admin_view_class):
        """ 
        使用 `~AdminSite.get_view_class` 创建 AdminView 类，并且返回 view 方法，可以用于 get_urls 方法中 
    ...
    def create_model_admin_view(self, admin_view_class, model, option_class):
        """ 
        使用 `~AdminSite.get_view_class` 创建 ModelAdminView 类，并且返回 view 方法，可以用于 get_urls 方法中 
            :param admin_view_class: AdminView 类，该类应该为 `~xadmin.views.base.ModelAdminView` 的子类 
    ...
    def urls(self):
        """ 
        返回 xadmin site 的urls，用于设置django的urls。该方法用于属性使用。
        在Django的 ``urls.py`` 中，使用示例如下:
            from django.conf.urls import patterns, include, url 
            import xadmin xadmin.autodiscover() 
            urlpatterns = patterns('', url(r'', include(xadmin.site.urls)), ) 
        """
...
# `AdminSite` 的单例，通常情况下可以直接使用这个 site，作为全站统一实例
site = AdminSite()
