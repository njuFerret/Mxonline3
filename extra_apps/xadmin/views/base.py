...
'''
Xadmin系统中主要的AdminView有：

BaseAdminView       :   所有AdminView的基础类，注册在该View上的插件可以影响所有的AdminView
CommAdminView       :   用户已经登录后显示的View，也是所有登陆后View的基础类。
...
选择好目标AdminView后就要在自己的插件中编写方法来修改或增强这些AdminView。其中每个AdminView可以拦截的方法及其介绍请参看各AdminView的文档。

关系图：https://blog.csdn.net/shaququ/article/details/77253252
...
def inclusion_tag(file_name, context_class=Context, takes_context=False):
    """
    为 AdminView 的 block views 提供的便利方法，作用等同于 `django.template.Library.inclusion_tag`
    """
    def wrap(func):
        @functools.wraps(func)
...
class BaseAdminObject(object):
    ...
    def get_model_view(self, view_class, model, *args, **kwargs):
        """
        获取 ModelAdminViewClass 的实例。首先通过 :class:`~xadmin.sites.AdminSite` 取得 model 对应的 OptionClass，然后调用 `get_view` 方法
...
class BaseAdminPlugin(BaseAdminObject):
    """
    所有 Plugin 的基类。继承于 :class:`BaseAdminObject` 。插件的注册和使用可以参看 `xadmin.sites.AdminSite.register_plugin` ，
...
class BaseAdminView(BaseAdminObject, View):
    ...
    而后您就可以在 ``me_test/`` 访问到该view了。当然xadmin同时提供了一些通用的 AdminView，分别为
    ...
    def as_view(cls):
        """
        复写了 `View.as_view` 方法，主要是将 `View.dispatch` 的也写到了本方法中，并且去掉了一些初始化操作，
        因为这些初始化操作在 AdminView 的初始化方法中已经完成了，可以参看 `BaseAdminView.init_request`
        """
    ...
    def init_plugin(self, *args, **kwargs):
        """
        AdminView 实例中插件的初始化方法，在 `BaseAdminView.init_request` 后调用。根据 AdminView 中
        的 base_plugins 属性将插件逐一初始化，既调用 `BaseAdminPlugin.init_request` 方法，并根据返回结果判断是否加载该插件。
...
class CommAdminView(BaseAdminView):
    ...
    **View属性**:
    ...
    def get_site_menu(self):
        ...
        `BaseAdminObject.get_model_perm` 和 `BaseAdminObject.get_model_url`。举例说明创建菜单::
    ...
    def get_model_icon(self, model):
        ...
        .. note::
            Model 图标，目前被使用在以下几个地方，当然您也可以随时使用在自己实现的页面中:
                ...
                * 添加、修改及删除页面的标题中
        ``FAQ: 如果定义 Model 图标``
        可以在 :class:`CommAdminView` 的 OptionClass 中通过 :attr:`CommAdminView.globe_models_icon` 属性设定全局的 Model 图标。
...
class ModelAdminView(CommAdminView):
    ...
    ModelAdminView 注册时使用 `xadmin.sites.AdminSite.register_modelview` 方法注册，具体使用实例可以参见该方法的说明，或参考实例::
    ...
    **实例属性**
        .. py:attribute:: opts              即 Model._meta
        .. py:attribute:: app_label         即 Model._meta.app_label
        .. py:attribute:: module_name       即 Model._meta.module_name
        .. py:attribute:: model_info        即 (self.app_label, self.module_name)
    """
    fields = None       #: (list,tuple) 默认显示的字段
    exclude = None      #: (list,tuple) 排除的字段，主要用在编辑页面
    ordering = None     #: (dict) 获取 Model 的 queryset 时默认的排序规则
    model = None        #: 绑定的 Model 类，在注册 Model 时，该项会自动附在 OptionClass 中，见方法 `AdminSite.register`
    remove_permissions = []
